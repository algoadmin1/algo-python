...
/** 
       ...
       userkeyinsert.php - adapted from: userNewsignup.php    Version __________    - w/ NEW geobytes.com ___                   ver 5.4
...
// 1d9a6ed0aa342400325b 

    // $slen=10;
    // $userkey1= random_bytes($slen);
    // // echo "'". bin2hex($bytes) . "',";
    // $userkey=bin2hex($userkey1) ;
...
// *********************************************
// ********************************************* start of insert loop
// *********************************************

            $k=0;
            for($k=0;$k<$MaxToInsert;$k++){


                $userkeysalt= substr(str_shuffle( $allchars  ),1,$keylength2);
                $userkeysalt2=bin2hex( random_bytes($keylength2) ) ;
                ...
                for($i=0;$i<$keylength;$i++){
                    ...
                    $rndcharpos = rand(0, $allchars_len1);  // ie 0..61 inclusive
                    $rndchar0   = substr($allchars, $rndcharpos,1 );  // T
                    $cumstr.=$rndchar0;

                }//for

                $userkey = $cumstr;
                $usersalt = $userkeysalt2;


               echo "  Inserting... [$userkey],  (alt =$userkeysalt ), userSALT = $userkeysalt2 <br />";
                    ...
                    $insertStr = "INSERT INTO userkeys (id,  useremail,  userkey,   usersalt, userIP ,
                            paidlevel, paidamount, paiduntildate, paiddate, paidtime,  timestamp )  VALUES ( 'NULL' , '$useremail', '$userkey' , '$usersalt', '$userIP', '$paidlevel', '$paidamount', '$paiduntildate' ,'$paiddate', '$paidtime', CURRENT_TIMESTAMP )";
...
// *********************************************
...
}else{   // if($useremail0 == "nil"){



        // do not insert - just check email input
        ...
        while($row = mysqli_fetch_array($result5))
        {
         ...
         if($id>0){
            echo "EMAILFOUND,$useremail0,$data_id,$data_key,$data_salt,$data_timestamp<br />";
         }else{
                        // SEE BELOW
                        // UPDATE and return $id = $data_id
                             // $query5u = "UPDATE `userkeys` SET `useremail` = `$useremail0`, `userIP` = '1.1.1.1', `paidlevel` = '2', `paidamount` = '100', `paiduntildate` = '2022-12-31', `paiddate` = '2022-09-30', `paidtime` = '20:01:13' WHERE `userkeys`.`id` = 16";
            ...
            // FIND a nil email...
            //                 
                    $query5e = "SELECT * FROM userkeys      WHERE    useremail LIKE 'nil' LIMIT 1";
                    $result5 = mysqli_query($con, $query5e); // or die("query failed ($query5e) - " . mysql_error());
            ...
            // UPDATE and return $id = $data_id
                            $query5u = "UPDATE userkeys SET useremail = '$useremail0', userIP = '111.9.1.1', paidlevel = '2', paidamount = '100', paiduntildate = '2022-12-31', paiddate = '2022-09-30', paidtime = '20:01:13' WHERE  id = '$id0' ";

                            $result5 = mysqli_query($con, $query5u); // or die("query failed ($query5u) - " . mysql_error());
                         
                            echo "EMAILUPDATE,$useremail0,$id0,$ukey<br />";
